...
\abstract{%
%
    　In computational fluid dynamics (CFD) of supersonic flows, shock-capturing methods smear discontinuities over several cells 
    due to numerical diffusion, leading to reduced accuracy in post-shock regions. In contrast, the shock-fitting method explicitly represents the shock front 
    by introducing shock-points and applying the Rankine–Hugoniot conditions, allowing accurate tracking of its position and shape 
    while minimizing artificial viscosity effects. In this study, the shock-fitting technique was implemented on a Cartesian grid to facilitate its application 
    to general geometries. Fundamental two-dimensional benchmark problems were analyzed to validate the method’s effectiveness. 
    Moreover, an automated procedure was developed to extract discontinuous regions from shock-capturing CFD results 
    and integrate them into the shock-fitting framework. 
    The proposed approach demonstrates the potential of combining shock detection and explicit front tracking 
    for accurate and efficient simulation of supersonic flows.
%
}
\begin{document}
\baselineskip=1zw
